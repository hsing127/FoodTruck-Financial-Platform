...
const SidebarLink = React.memo(({ href, icon: Icon, label, isCollapsed }: SidebarLinkProps) => {
  const path = usePathname();
  const isActive = useMemo(() => path === href || href === "/dashboard", [path, href]);

  return (
    <Link href={href} aria-label={label}>
      <motion.div
        className={`cursor-pointer flex items-center ${isCollapsed ? "justify-start pl-5" : "pl-5"} py-4 hover:text-blue-500 hover:bg-blue-100 gap-3 transition-colors ${isActive ? "bg-blue-200 text-gray-900" : ""}`}
...
const Sidebar = React.memo(() => {
  ...
  return (
    <div className={sidebarClassNames}>
      <div className={`flex gap-3 justify-between md:justify-normal items-center pt-8 ${isSidebarCollapsed ? "pl-3" : "pl-3"}`}>
        <motion.button
